...
import { NodeSendVideoData } from "../Payload";
...
export const NodeSendVideo = (props: PropsNodeReply): Promise<void> =>
  new Promise(async (res, rej) => {
    ...
    const urlStatic = resolve(__dirname, `../../../../static/video`);
    ...
    try {
      await SendVideo({
        connectionId: props.connectionId,
        toNumber: numberLead,
        video: readFileSync(urlStatic + "/" + nameFile.name),
        mimetype: "video/mp4",
        caption: newMessage,
      });
    } catch (error) {
