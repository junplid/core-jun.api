...
import { NodeWebformData } from "../Payload";
import phone from "libphonenumber-js";
interface PropsNodeWebform {
  data: NodeWebformData;
...
export const NodeWebform = (props: PropsNodeWebform): Promise<void> => {
  return new Promise(async (res, rej) => {
    const { contactsWAOnAccountId, data } = props;

    const alreadyExistsVarHeaders: boolean = onThereVariable(data.headers);
    const alreadyExistsVarUrl: boolean = onThereVariable(data.url);

    let variables: Variable[] = [];

    if (alreadyExistsVarHeaders || alreadyExistsVarUrl) {
    ...
    if (variables.length) {
      const newHeaders = await onUpdateString(variables, data.headers);
      const newUrl = await onUpdateString(variables, data.url);
      axios[data.method](newUrl!, {
        ...(newHeaders && { headers: json5.parse(newHeaders) }),
      });
      res();
      return;
    } else {
      axios[data.method](data.url!, {
        ...(data.headers && { headers: json5.parse(data.headers) }),
      });
      res();
      return;
    }
