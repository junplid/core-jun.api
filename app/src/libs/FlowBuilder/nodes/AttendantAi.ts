...
import { NodeAttendantAIData } from "../Payload";
...
const saveVariableOnContactLead = async (props: {
  id: number;
  value: string;
  accountId: number;
  contactsWAOnAccountId: number;
  ...
  for await (const propss of businessIdsOnVariable) {
    if (!propss.ContactsWAOnAccountVariableOnBusiness.length) {
      ...
      for await (const { id } of propss.ContactsWAOnAccountVariableOnBusiness) {
        const alreadyExists =
...
const baseURLsAI: { [x in TypeArtificialIntelligence]: string | undefined } = {
  DEEPSEEK: "https://api.deepseek.com",
  OPENAI: undefined,
};

const modelsAI: { [x in ModelArtificialIntelligence]: string } = {
...
export const NodeAttendantAI = (
  ...
  return new Promise<TStatusReturn>(async (res, rej) => {
    ...
    const attendantAI = await prisma.attendantOnAI.findFirst({
      where: { id: props.data.attendantAI, accountId: props.accountId },
      select: {
        name: true,
        personality: true,
        briefing: true,
        ...
        ArtificialIntelligence: {
          select: { apiKey: true, type: true, model: true },
        },
