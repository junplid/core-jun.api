...
export class UpdateFunnelKanbanTicketForSelectImplementation
  implements UpdateFunnelKanbanTicketForSelectRepository_I
{
  constructor(
  ...
  async fetchTicketExist(props: { ticketId: number }): Promise<boolean> {
    try {
  ...
  async fetchTicket(props: {
    ...
    try {
      const data = await this.prisma.stepsFunnelKanbanOnTickets.findFirst({
      ...
      return data
        ? { sequence: data.sequence, oldColumnId: data.StepsFunnelKanban.id }
        : null;
    } catch (error) {
      console.log(error);
      throw new Error("Erro `Get Tag Asset Data`.");
