...
import {
  CreateCampaignOndemandRepository_I,
  PropsCreateCampaignOndemand,
} from "./Repository";

export class CraeteCampaignImplementation
  implements CreateCampaignOndemandRepository_I
{
  constructor(
  ...
  async createCampaignOndemand({
    businessIds,
    audienceId,
    ...data
  }: PropsCreateCampaignOndemand): Promise<{
    readonly id: number;
      ...
      const { CampaignOnBusiness, ...rest } = await this.prisma.campaign.create(
        {
          data: {
            ...
            AudienceOnCampaign: { create: { audienceId } },
          },
          select: {
            id: true,
            createAt: true,
