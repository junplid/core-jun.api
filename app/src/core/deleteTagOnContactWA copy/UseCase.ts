...
import { DeleteTagOnContactWADTO_I } from "./DTO";

export class DeleteTagOnContactWAUseCase {
  constructor() {}

  async run({ ...dto }: DeleteTagOnContactWADTO_I) {
    ...
    if (!existTagOnContact) {
      return {
        message: "Tag já foi removida do contato.",
        status: 200,
      };
    }

    await prisma.tagOnContactsWAOnAccount.delete({
      where: { id: existTagOnContact.id },
    });
