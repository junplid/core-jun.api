...
import { CollectRouteOrderDTO_I } from "./DTO";

export class CollectRouteOrderUseCase {
  constructor() {}

  async run(dto: CollectRouteOrderDTO_I) {
    const getRouter = await prisma.deliveryRouter.findFirst({
      ...
      select: {
        ...
        DeliveryRouterOnOrders: {
          ...
          where: { Order: { n_order: dto.n_order } },
        },
      },
    });
    ...
    socketOrders.update_order({ id: orderId, status: "on_way" }, []);
    ...
    const nextNode: any = nextEdgesIds?.find((nd: any) =>
      nd.sourceHandle?.includes("on_way"),
    );
